...
import { deleteCompensationFunds } from "../../../functions/accounting/compensationFunds";
...
const DeletingCompensationFunds = ({ fetchPurchases, id, setShowModal, setResponse }: Props) => {


    const deletePurchases = async () => {
        if ((await deleteCompensationFunds(id)).affectedRows > 0) {
            setResponse("Fonds d'indemnisation supprimé !");
            setShowModal(false);
            fetchPurchases();
        } else {
            alert("Erreur");
...
export default DeletingCompensationFunds;