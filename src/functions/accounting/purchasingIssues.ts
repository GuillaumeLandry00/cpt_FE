 ...
 */
export const getPurchasingIssues = async (): Promise<unknown> => {
    try {
        //We make the request
        const response = await axios.get<IPurchasingIssues[]>(BASE_URL + "accounting/purchasingIssues/", { headers: { "x-access-token": localStorage.getItem('token') as string } });
